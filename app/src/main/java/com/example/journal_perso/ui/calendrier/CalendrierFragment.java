        ...
        cal.setOnDateChangeListener(new CalendarView.OnDateChangeListener() {
            ...
            public void onSelectedDayChange(@NonNull CalendarView view, int year, int month, int dayOfMonth) {
                ...
                for (int i = 0; i < finalData.getMesEspaces().size(); i++) {
                    if (finalData.getMesEspaces().get(i).getListJour().contains(dayOfMonth)) {
                        LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                        Button nB = new Button(getContext());
                        nB.setLayoutParams(p);
                        nB.setHint("Texte");
                        ll.addView(nB);
                      /* Toast patate = Toast.makeText(getContext(), finalData.getMesEspaces().get(i).getNom(), Toast.LENGTH_LONG);
                       patate.show();*/
                    }
                }
            }
